...
export default function Index(props: PropsType) {
  ...
  const [invitation, { loading }] = useInvitationTeachersMutation({
  ...
  const handleInvitation = () => {
    ...
    invitation({
      ...
      update: (cache, result) => {
        if (result.data) {
          const { invitationTeachers } = result.data;
          cache.writeQuery<
            TeacherApplicationsQuery,
            TeacherApplicationsQueryVariables
          >({
            query: TeacherApplicationsDocument,
            data: { teacherApplications: invitationTeachers },
          });
        }
      },
    });
