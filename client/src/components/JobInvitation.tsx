...
import jobInvitationMutate from "./jobInvitationMutate";
...
export default function JobInvitation() {
  ...
  const { data } = useJobRequestQuery();
  useApplicatonUpdatedSubscription({
    onSubscriptionData: (tData) => {
      if (tData.subscriptionData.data) {
        const newItem = tData.subscriptionData.data.applicationUpdated;
        if (data) {
          if (data.jobRequests.length > 0) {
            const otherS = data.jobRequests.filter(
              (j) => j._id !== newItem._id
            );

            tData.client.writeQuery<JobRequestQuery, JobRequestQueryVariables>({
            ...
            tData.client.writeQuery<JobRequestQuery, JobRequestQueryVariables>({
              query: JobRequestDocument,
              data: {
                jobRequests: [{ ...newItem, __typename: "TeacherApplication" }],
              },
            });
          }
        }
  ...
  return (
    <>
      <DropDown
        ...
        button={
          <IconButton onClick={() => setDisplayMenu((o) => !o)}>
            <Badge
              badgeContent={data && data.jobRequests.length}
              color="secondary"
            >
      ...
      >
        <DropDownList title="İş İstekleri">
          {data &&
          ...
          {data && data.jobRequests.length === 0 && (
